...
Given("Test search works", { timeout: 30000 }, async function () {
    let driver = new Builder().forBrowser("chrome").build();
    await driver.get("http://localhost:3000/workerhome");
    await driver.findElement(By.id("srchwrk")).sendKeys("wa");
    await driver.wait(until.elementLocated(By.id("srchfrm")), 30000);
    expect(await driver.wait(until.elementLocated(By.id("srchfrm"))));
